...
import com.google.ar.core.examples.kotlin.common.rendering.ShaderUtil.checkGLError
...
class PlaneRenderer {
    ...
    private var vertexBuffer: FloatBuffer = ByteBuffer.allocateDirect(
        ...
        .asFloatBuffer()
    private var indexBuffer: ShortBuffer = ByteBuffer.allocateDirect(
    ...
    private fun updatePlaneParameters(
        ...
        // out ok.
        val xScale = max(
            ((extentX - 2 * FADE_RADIUS_M) / extentX).toDouble(),
            0.0
        ).toFloat()
        val zScale = max(
            ((extentZ - 2 * FADE_RADIUS_M) / extentZ).toDouble(),
            0.0
        ).toFloat()
    ...
    fun drawPlanes(allPlanes: Collection<Plane>, cameraPose: Pose, cameraPerspective: FloatArray) {
        ...
        for (sortedPlane in sortedPlanes) {
            ...
            planeAngleUvMatrix[0] = +cos(angleRadians.toDouble()) as Float * uScale
            planeAngleUvMatrix[1] = -sin(angleRadians.toDouble()) as Float * vScale
            planeAngleUvMatrix[2] = +sin(angleRadians.toDouble()) as Float * uScale
            planeAngleUvMatrix[3] = +cos(angleRadians.toDouble()) as Float * vScale
